...
#Load hypocenter
hypo = gix.loadCSV('./data/kaikoura_hypo.txt')
...
ax.scatter(hypo[0], hypo[1], zorder=4, s=120, marker='*', edgecolors='b', facecolors='b')
...
axins.set_ylim([5215093.467, 5409948.853])
for i in range(len(fault_geometry)):
    axins.plot(fault_geometry[i][:, 0], fault_geometry[i][:, 1], color=[0.6, 0.6, 0.6], linewidth=1.25, zorder=2)

h = axins.scatter(hypo[0], hypo[1], s=180, zorder=4, marker='*', edgecolors='b', facecolors='b')
axins.grid()
...
ax.legend([rup, fa, h, st2, st1], ['Rupture geometry', 'ML-considered sources', 'Hypocenter', 'Immediate availability', 'Delayed availability'], loc=4)

plt.tight_layout()

plt.savefig('./kaikoura_Ham17.pdf', dpi=600)
