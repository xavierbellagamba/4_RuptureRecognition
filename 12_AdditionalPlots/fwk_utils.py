...
def saveFaultMap(P_fault, fault_name, str_file):
	...
	im = plt.scatter(P_select, P_select, c=P_select, cmap=cmap, vmin=1., vmax=2.01)#P_max+0.01)
	...
	cb = plt.colorbar(im, orientation='horizontal', cax=cax, ticks=np.arange(1., 2.+0.01, (2.-1.)/5.))#P_max+0.01, (P_max-1.)/5.))
...
def saveDiscriminatorResults(P_rupture, P_fault, rupture_dict, fault_dict, data_name, savePlot):
	...
	if savePlot:
		...
		if not os.path.exists(dir_path):
			os.mkdir(dir_path)
		saveFaultMap(P_fault, fault_name, './gen/' + data_name + '/plot/P_fault.pdf')
...
def comparePredictObsComplex(data_name, realID, discriminator_GM, generator_GM, fault_dict, rupture_dict, IM_name, saveSHP, savePlot):
	...
	i_rpt = np.argwhere(P_rupture[0, 0, 0, :] > 0.0061)
